function InvokeLiveShow {
    ...
    if ($Toggles.Animation) {
        ...
        $fortune = GetFortune -Database $Config.fortune.database
        $mode = $animationModes | Get-Random

        $baseCow = InvokeCowsay -Text $fortune -CowFile $cowName
        ...
        if ($Toggles.Lolcat) {
            ...
            $cowText = FormatLolcat -Text $cowText -Seed $lolSeed
        }
        ...
        $payload = @{
            ...
            duration   = 0
            fps        = 30
        } | ConvertTo-JsonSafe -Compress
        $result = $payload | InvokeEngine
        if ($null -eq $result) {
            Write-Host $fortune
            Write-Host $cowText
        }
